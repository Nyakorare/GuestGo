...
// Handle GET requests to completion email endpoint (for testing)
app.get('/api/send-completion-email', (req, res) => {
  res.status(405).json({ 
    error: 'Method not allowed',
    message: 'This endpoint only accepts POST requests. Use POST to send emails.',
    method: 'POST',
    endpoint: '/api/send-completion-email'
  });
});

// Visit completion email endpoint
app.post('/api/send-completion-email', async (req, res) => {
  try {
    ...
    console.log('📧 Attempting to send completion email:', {
      to: visitorEmail,
      from: `${fromName} <${fromEmail}>`,
      subject: `Thank You for Your Visit - ${visitId} | GuestGo`,
    });
...
app.listen(PORT, () => {
  ...
  console.log(`   Completion email endpoint: http://localhost:${PORT}/api/send-completion-email\n`);
});
