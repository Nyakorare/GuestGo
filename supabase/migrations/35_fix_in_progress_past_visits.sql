-- Fix in_progress visits that are past their visit date
-- This migration updates the status update function to also handle 'in_progress' status visits
-- that should be marked as completed_flagged or unsuccessful when their date has passed

-- Update the update_visit_statuses_server function to handle in_progress status
...
-- Also update the fix_pending_past_visits function to handle in_progress status
...
-- Also update the force_update_visit_by_email function to handle in_progress status
...
BEGIN
    ...
    -- Only process if status is pending or in_progress
    IF visit_record.status NOT IN ('pending', 'in_progress') THEN
        RETURN result_text || 'Visit is not pending or in_progress, current status: ' || visit_record.status;
    END IF;
...
-- Also update the force_update_all_pending_past_visits function to handle in_progress status
...
BEGIN
    ...
    result_text := result_text || 'Processing pending and in_progress past visits...' || E'\n';
    result_text := result_text || 'Current date: ' || philippine_date || E'\n';
    result_text := result_text || 'Gate fields exist: ' || gate_fields_exist || E'\n';
