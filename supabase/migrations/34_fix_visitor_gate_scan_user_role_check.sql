-- Fix scan_gate_entrance_with_face to handle missing user_roles records
-- This migration fixes the issue where visitors without a user_roles record
-- cannot scan gate entrances. It will create a default visitor role if missing.
...
BEGIN
    ...
    BEGIN
        SELECT role INTO STRICT user_role_record FROM user_roles WHERE user_id = p_scanned_by;
        user_role_value := user_role_record.role;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            -- If no role record exists, create a default visitor role
            INSERT INTO user_roles (user_id, role)
            VALUES (p_scanned_by, 'visitor')
            ON CONFLICT (user_id) DO UPDATE SET role = EXCLUDED.role;
            
            -- Set to visitor role
            user_role_value := 'visitor';
        WHEN TOO_MANY_ROWS THEN
            -- Should not happen due to UNIQUE constraint, but handle it
            SELECT role INTO user_role_record FROM user_roles WHERE user_id = p_scanned_by LIMIT 1;
            user_role_value := user_role_record.role;
    END;
    
    -- Check if user is a visitor
    IF user_role_value != 'visitor' THEN
...
COMMENT ON FUNCTION scan_gate_entrance_with_face IS 'Scans a gate entrance with face detection data, supporting temporary_exit re-entry logic and auto-creating visitor role if missing';
